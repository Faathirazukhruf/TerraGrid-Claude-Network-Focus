import { Contact, QualityFlags, MissingGoalContactAlert, AmbiguousNameAlert, ThinContextAlert, StaleContactAlert, StrategicGoal } from './types.js';
...
export function detectQualityFlags(
  ...
  const missingGoalContacts: MissingGoalContactAlert[] = [];
  const ambiguousNames: AmbiguousNameAlert[] = [];
  const thinContextContacts: ThinContextAlert[] = [];
  const staleContacts: StaleContactAlert[] = [];
  ...
  const firstNameMap = new Map<string, { contact: Contact; row: number }[]>();
  const fullNameMap = new Map<string, { contact: Contact; row: number }[]>();
