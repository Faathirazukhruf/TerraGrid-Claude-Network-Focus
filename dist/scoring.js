...
exports.matchGoals = matchGoals;
exports.calculateScores = calculateScores;
exports.scoreAllContacts = scoreAllContacts;
const goals_js_1 = require("./goals.js");
function matchGoals(contact, goals = goals_js_1.TERRAGRID_GOALS) {
    const matches = [];
...
function calculateScores(contact) {
    ...
    const isHighLeverage = goalAlignmentScore >= 40 &&
...
function generateRecommendationAndRisk(contact, primaryGoalId) {
    const notesLower = `${contact.lastContactSummary} ${contact.notes}`.toLowerCase();
    const tagsLower = contact.tags.map(t => t.toLowerCase());
    ...
    if ((notesLower.includes('offered to help') || notesLower.includes('cto search') || notesLower.includes('strong candidates')) &&
    ...
    if (contact.daysSinceLastContact >= 60 &&
...
function scoreAllContacts(contacts) {
